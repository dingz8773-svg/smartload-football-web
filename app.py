...
def render_advice_text(ctx):
    ...
    plan = build_plan(ctx)
    band = ctx.get("band", "green")
    head = f"今日 FRI {ctx.get('FRI', float('nan')):.2f}（{band}）— " + ("；".join(reasons) if reasons else "未见显著异常") + "。"
    text = (
        head + "\n"
...
with left:
    ...
    pid_adv = st.selectbox("选择球员", sorted(df["player_id"].unique()), index=0, key="adv_pid")
    date_adv = st.selectbox("选择日期", all_dates, index=len(all_dates)-1,
