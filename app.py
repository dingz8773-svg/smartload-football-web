...
# ---------- Fonts: make Chinese render properly ----------
# Try several common Chinese fonts; fallback to default if not found.
matplotlib.rcParams["font.sans-serif"] = ["SimHei", "Microsoft YaHei", "Noto Sans CJK SC", "Source Han Sans SC", "Heiti SC", "Arial Unicode MS", "DejaVu Sans"]
matplotlib.rcParams["axes.unicode_minus"] = False  # make minus sign show correctly with Chinese fonts
...
with st.sidebar:
    ...
    st.markdown("---")
    st.subheader("阈值设置")
    acwr_th = st.number_input("ACWR 风险阈值", value=1.5, step=0.05, format="%.2f")
    y_th = st.number_input("FRI · 黄灯阈值（≥）", value=0.5, step=0.05, format="%.2f")
    r_th = st.number_input("FRI · 红灯阈值（≥）", value=0.7, step=0.05, format="%.2f")
    recompute = st.checkbox("按阈值从 FRI 重算 band（忽略原 band）", value=False)
    st.caption("提示：若原数据无 band 或阈值不同，可勾选此项让清单按照上面的阈值重算。")
...
def read_df(file) -> pd.DataFrame:
    ...
    if name.endswith(".csv"):
        ...
        for enc in ("utf-8-sig","gbk"):
            try:
                import io as _io
                return pd.read_csv(_io.BytesIO(data), encoding=enc)
            except Exception:
                continue
        raise ValueError("CSV 编码无法识别（尝试 UTF-8-SIG 与 GBK 失败）")
    else:
        data = json.load(file)
...
with left:
    ...
    fri = df.pivot_table(index="player_id", columns=df["date"].dt.date, values="FRI", aggfunc="mean").sort_index()
    if not fri.empty:
        fig, ax = plt.subplots()
...
# ===== 右：个体趋势（折线+圆点 + 阈值虚线 & 中文图例） =====
def _xtick_format(ax, dates):
    # Reduce tick density: show ~8 ticks at most
    n = len(dates)
    if n <= 8:
        idx = range(n)
    else:
...
sel_date = st.selectbox("选择日期", all_dates, index=len(all_dates)-1, format_func=lambda d: d.strftime("%Y-%m-%d"))
day_df = df[df["date"].dt.date == sel_date]
alerts = day_df[day_df["band"].isin(["yellow","red"])][["player_id","date","FRI","band","ACWR_7_28","HRV_ratio","CMJ_change_pct"]]\
...
if alerts.empty:
    ...
    st.download_button("下载 CSV", data=csv_bytes, file_name=f"alerts_{sel_date}.csv", mime="text/csv")
